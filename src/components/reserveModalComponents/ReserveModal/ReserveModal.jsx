...
import {skl} from '../../../utils'

import classes from './reserveModal.module.scss'
...
const ReserveModal = () => {
	...
	const reserveAmount = useSelector( state => state.reserveModal.reserveModalAmount );
	const city = useSelector( state => state.reserveModal.reserveModalData[0].city)
	const isAllowed = useSelector( state => state.reserveModal.reserveModalData[0].isAllowed)
	
	const countWrapper = document.getElementById( 'reserve_count_wrapper' )
	
	useEffect( () => {
		if (reserveAmount > 0) countWrapper.innerHTML = `<div class="round_count" id="reserve_count">${reserveAmount}</div>`
		
	}, [reserveAmount, countWrapper] )
	...
	useStopScrollJump(showReserveModal)
	...
	return (
		<Modal className={classes['modal']} onClick={handleOutsideClick}>
			<div className={classes['reserve-modal']} style={!isAllowed ? {maxWidth: '650px'} : null}>
				...
				<h2 className={classes['reserve-modal__title']}>
						<span>
							<svg className={classes['reserve-modal__titleArrow']} width="15" height="13" viewBox="0 0 15 13"
									 fill="none"
					...
					<span>Товар в&nbsp;наличии в&nbsp;{shops.length}&nbsp;{shop} г.&nbsp;{city}</span>
				</h2>
				<ReserveTableItems/>
				<ReserveItemsMobile/>
			</div>
